...
- **Token 传递方式**: HTTP Header: `satoken: <token值>`
- **公开接口**: 商品列表、商品详情、分类列表（无需登录）
- **需要登录**: 发布商品、编辑商品、删除商品、我的商品
...
| minPrice | number | ❌ | 最低价格 | 100 |
| maxPrice | number | ❌ | 最高价格 | 1000 |
| sortField | string | ❌ | 排序字段：price/view_count/create_time | price |
| sortOrder | string | ❌ | 排序方式：asc/desc | asc |
| pageNum | number | ❌ | 页码，默认1 | 1 |
| pageSize | number | ❌ | 每页条数，默认20 | 20 |
...
| 参数名 | 类型 | 必填 | 说明 | 示例 |
|--------|------|------|------|------|
| title | string | ✅ | 商品标题，最多100字符 | 二手iPhone 13 |
| description | string | ✅ | 商品描述，最多2000字符 | 9成新，无磕碰 |
| price | number | ✅ | 当前价格，0.01-999999.99 | 4999.00 |
| originalPrice | number | ❌ | 原价，0.01-999999.99 | 6999.00 |
| categoryId | number | ✅ | 分类ID | 1 |
| imageUrls | array | ✅ | 图片URL数组，1-9张 | ["url1", "url2"] |
...
const handleSubmit = async () => {
  ...
  if (!form.title || form.title.length > 100) {
    ElMessage.warning('请输入商品标题（不超过100字）');
    return;
  }
  if (!form.description) {
  ...
  if (!form.price || form.price < 0.01 || form.price > 999999.99) {
    ElMessage.warning('请输入正确的价格（0.01-999999.99）');
    return;
  }
  if (!form.categoryId) {
...
|--------|------|------|------|------|
| id | number | ✅ | 商品ID | 123 |
| title | string | ✅ | 商品标题，最多100字符 | 二手iPhone 13（降价） |
| description | string | ✅ | 商品描述，最多2000字符 | 9成新，无磕碰 |
| price | number | ✅ | 当前价格，0.01-999999.99 | 4500.00 |
| originalPrice | number | ❌ | 原价，0.01-999999.99 | 6999.00 |
| categoryId | number | ✅ | 分类ID | 1 |
| imageUrls | array | ✅ | 图片URL数组，1-9张 | ["url1", "url2"] |
...
interface ProductListVO {
  ...
  status: number;          // 状态：1-已发布，2-已售出，3-已下架
  ...
  <div class="product-list">
    ...
    <el-form :inline="true" class="search-form">
      <el-form-item label="关键词">
        <el-input v-model="searchForm.keyword" placeholder="搜索商品标题或描述" clearable />
      </el-form-item>
...
**A**: 可能原因：
1. 数据库没有商品数据 → 执行初始化脚本 `init_product_data.sql`
2. 筛选条件过严 → 重置筛选条件
3. 商品状态不是1（已发布） → 检查商品状态

### Q2: 发布商品时报401错误？

**A**: 未登录或 Token 失效
1. 检查 localStorage 中是否有 `satoken`
2. 检查请求头是否正确携带 Token
3. Token 过期需要重新登录
